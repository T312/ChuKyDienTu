...
namespace ChuKyDienTu
{
    public class ChuKyDienTu
    {
        ...
        {
            SHA256Managed managed = new SHA256Managed(); // khởi tạo method hash
            byte[] bytes = Encoding.UTF8.GetBytes(mess); // trả về mảng byte
            bytes = managed.ComputeHash(bytes);
            StringBuilder builder = new StringBuilder(); // chuyển mảng byte về dạng chuỗi
